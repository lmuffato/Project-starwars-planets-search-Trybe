      ...
      filters.filterByNumericValues.forEach((filter) => {
        ...
        case 'maior que':
          filteredPlanets = filteredPlanets
            .filter((el) => el[column] > parsedValue); break;
        case 'igual a':
          filteredPlanets = filteredPlanets.filter((el) => el[column] === value); break;
  ...
  const createRows = () => {
    if (planets) {
      let filteredByName = planets
        .filter((x) => x.name.includes(filters.filterByName.name));
      if (filters.filterByNumericValues.length > 0) {
